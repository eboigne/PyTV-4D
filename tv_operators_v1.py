...
def D(img, reg_z_over_reg = 0, reg_time = 0, halve_tv_at_both_end = False, factor_reg_static = 0, mask_static = False):
    '''
    ...
    Returns: 
        out: D(img) of dimensions Nz x 4/6/8 x M x N x N # TODO: optimize by shuffling dimensions around.
    '''
...
#     D_img[0,:-1,:] = -  D_img[2,:-1,:] # The row_diff at the last row is 0

    # The intensity differences across columns (Upwind / Forward)
    D_img[:,1,:,:,:-1] = img[:,:, :, 1:] - img[:,:, :, :-1]
#     D_img[1,:,:-1] = -  D_img[3,:,:-1] # The col_diff at the last col is 0

    # The intensity differences across rows (Downwind / Backward)
    D_img[:,2,:,1:,:] = img[:, :, :-1, :] - img[:, :, 1:, :] # The row_diff at the first row is 0

    # The intensity differences across columns (Downwind / Backward)
    D_img[:,3,:,:,1:] = img[:,:, :, :-1] - img[:,:, :, 1:] # The col_diff at the first col is 0
...
def D_T(img, reg_z_over_reg = 0, reg_time = 0, halve_tv_at_both_end = False, factor_reg_static = 0, mask_static = False):
    '''
...
def D_downwind(img, reg_z_over_reg = 0, reg_time = 0):
    ...
    Parameters:
        img : img of dimensions Nz x M x N x N
    Returns: 
        out: D(img) of dimensions Nz x 2/3/4 x M x N x N # TODO: optimize by shuffling dimensions around.
    '''
    if reg_z_over_reg == np.nan:
        reg_z_over_reg = 0.0
...
def D_upwind(img, reg_z_over_reg = 0, reg_time = 0):
    '''
    Calculates the image of the operator D (gradient discretized using centered scheme) applied to variable img
    Parameters:
        img : img of dimensions Nz x M x N x N
